...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        try {
            final File imageFile = configuration.diskCache.get(uri);
            if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
                ...
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        imageLoadingInfo.listener.onLoadingComplete(uri, null, null, imageFile);
                    }
                });
            } else {
                L.d(LOG_LOAD_IMAGE_FROM_NETWORK, memoryCacheKey);
                loadedFrom = LoadedFrom.NETWORK;
                boolean successDoCacheImageOnDisk = tryCacheImageOnDisk();
                if (successDoCacheImageOnDisk) {
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            imageLoadingInfo.listener.onLoadingComplete(uri, null, null, imageFile);
